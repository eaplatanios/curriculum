...
/** Efficient streaming histogram implementation based on an algorithm described in
  ...
  * The histogram consumes numeric samples and maintains a running approximation of the samples
  * distribution using the specified maximum number of bins.
  *
  * This implementation is based on this [Java/Clojure implementation by Adam Ashenfelter](
...
object Histogram {
  ...
  case class Bin(mean: Double) extends Ordered[Bin] {
    ...
    override def compare(that: Bin): Int = {
      Ordering.Double.IeeeOrdering.compare(mean, that.mean)
    }
  }
  ...
  /** This bin reservoir implements bin operations (insertions, merges, etc.), using an underlying array buffer. It is
    ...
    * respect to the number of bins in the histogram. For histograms with more bins, the `TreeBinReservoir` class
    * offers better performance.
    *
    * @param  maxNumBins                Maximum number of bins.
    * @param  useNumSamplesWeightedGaps Boolean value indicating whether to weigh the gaps between bins by the number
    *                                   of samples in those bins.
  ...
  /** This bin reservoir implements bin operations (insertions, merges, etc.), using an underlying tree map. It is
    * best used for histograms with a large (i.e., `> 256`) number of bins. It has O(log(N)) insertion performance
    * with respect to the number of bins in the histogram. For histograms with fewer bins, the `ArrayBinReservoir`
    * class offers better performance.
    *
    * @param  maxNumBins                Maximum number of bins.
    * @param  useNumSamplesWeightedGaps Boolean value indicating whether to weigh the gaps between bins by the number
    *                                   of samples in those bins.
  ...
  class TreeBinReservoir(
    ...
    override def floor(value: Double): Option[Bin] = {
      _bins.rangeTo(value).lastOption.map(_._2)
    }

    override def ceil(value: Double): Option[Bin] = {
      _bins.rangeFrom(value).headOption.map(_._2)
    }
  ...
  object TreeBinReservoir {
    case class Gap(startBin: Bin, endBin: Bin, space: Double) extends Ordered[Gap] {
      override def compare(that: Gap): Int = {
        var result = Ordering.Double.IeeeOrdering.compare(space, that.space)
        if (result == 0)
